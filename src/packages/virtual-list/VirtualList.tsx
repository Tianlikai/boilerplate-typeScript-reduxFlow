...
import { ItemStyle, VListProps, ItemPosition } from "./interface";
 ...
 * @param omitKeys
 */
const omitProps = <T extends object, U extends keyof T>(
  target: T,
  omitKeys: U[],
...
export class VirtualList extends React.PureComponent<VListProps, State> {
  static defaultProps = {
    overscanCount: 3,
  ...
  itemSizeGetter = (itemSize: VListProps["itemSize"]) => {
  ...
  componentWillReceiveProps(nextProps: VListProps) {
    const {
      estimatedItemSize,
      itemCount,
      itemSize,
  ...
  componentDidUpdate(_: VListProps, prevState: State) {
  ...
  private getSize(index: number, itemSize: ItemSize) {
